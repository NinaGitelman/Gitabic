#include "AESHandler.h"

bool AESHandler::randInit = false;

const array<uint8_t, 256>
AESHandler::mul2 = {
    
};

const array<uint8_t, 256> AESHandler::mul3 = {
    
};

const array<uint8_t, 256> AESHandler::mul9 = {
    
};

const array<uint8_t, 256> AESHandler::mul11 = {
    
};

const array<uint8_t, 256> AESHandler::mul13 =
{
    
};

const array<uint8_t, 256> AESHandler::mul14 =
{
    
};

AESHandler::AESHandler() {
    // Init the random seed one time
    if (!randInit) {
        randInit = true;
        srand(time(0));
    }
    // Generate random key and expands it
    keyExpansion(generateRandomKey());
}

AESHandler::AESHandler(AESKey key) {
    // Init the random seed one time
    if (!randInit) {
        randInit = true;
        srand(time(0));
    }
    // Expands the key
    keyExpansion(key);
}

Matrix4x4 AESHandler::encrypt(vector<uint8_t> &data, bool toPad, bool cbc) {
    if (!toPad && data.size() % BLOCK != 0) {
        throw std::runtime_error("Vector size must be padded or 16 bytes multiplication");
    }
    if (toPad) {
        padData(data);
    }
    Matrix4x4 iv = generateRandomMat();
    Matrix4x4 lastMat = iv; // Keep track of the former block for cbc mode
    for (size_t i = 0; i < data.size(); i += BLOCK) // Encrypt each block
    {
        Matrix4x4 &mat = convertToMat(data.data() + i);
        if (cbc) // Xor with previous / iv for avoiding patterns
        {
            xorEqual(mat, lastMat);
        }
        encryptMat(mat);
        lastMat = mat;
    }
    return iv;
}

void AESHandler::decrypt(vector<uint8_t> &data, Matrix4x4 *iv, bool isPadded) {
    if (data.size() % BLOCK != 0) {
        throw std::runtime_error("Vector size must be 16 bytes multiplication");
    }
    for (int i = data.size() - BLOCK; i >= 0; i -= BLOCK) {
        Matrix4x4 &mat = convertToMat(data.data() + i);
        decryptMat(mat);
        if (iv != nullptr) // Reverse cbc if needed
        {
            xorEqual(mat, i >= BLOCK ? convertToMat(data.data() + i - BLOCK) : *iv);
        }
    }
    if (isPadded) {
        dePadData(data);
    }
}

Matrix4x4 AESHandler::generateRandomMat() {
    Matrix4x4 random;
    for (auto &it: random) {
        for (auto &it2: it) {
            it2 = rand() % 256;
        }
    }
    return random;
}

AESKey AESHandler::generateRandomKey() {
    AESKey random;
    for (auto &it: random) {
        it = rand() % 256;
    }
    return random;
}

AESKey AESHandler::getKey() {
    return keys[0];
}

void AESHandler::printHexDump(const Matrix4x4 &matrix) {
    for (const auto &row: matrix) {
        for (uint8_t byte: row) {
            std::cout << std::hex << std::setw(2) << std::setfill('0')
                    << static_cast<int>(byte) << " ";
        }
        std::cout << std::endl;
    }
}

void AESHandler::printHexDump(const AESKey &key) {
    for (const auto &byte: key) {
        std::cout << std::hex << std::setw(2) << std::setfill('0')
                << static_cast<int>(byte);
    }
    std::cout << std::endl;
}

uint8_t AESHandler::rcon(uint8_t round) {
    static const uint8_t RconTable[11] = {
        0x00, // Not used
        0x01, 0x02, 0x04, 0x08,
        0x10, 0x20, 0x40, 0x80,
        0x1B, 0x36
    };

    if (round < 1 || round > R) {
        throw std::out_of_range("Invalid round for Rcon");
    }
    return RconTable[round];
}

uint8_t AESHandler::sBox(uint8_t input) {
    static const uint8_t SBoxTable[256] = {
        
    };

    return SBoxTable[input];
}

uint8_t AESHandler::sBoxInv(uint8_t input) {
    static const uint8_t SBoxInvTable[256] = {
        
    };

    return SBoxInvTable[input];
}

void AESHandler::rotate(Word &word, int8_t by) {
    Word temp;
    for (size_t i = 0; i < word.size(); i++) {
        temp[i] = word[(i + by + 4) % 4];
    }
    memcpy(word.data(), temp.data(), word.size());
}

void AESHandler::subWord(Word &word) {
    for (size_t i = 0; i < word.size(); i++) {
        word[i] = sBox(word[i]);
    }
}

void AESHandler::subBytes(Matrix4x4 &mat, bool isEnc) {
    for (size_t i = 0; i < mat.size(); i++) {
        for (size_t j = 0; j < mat[i].size(); j++) {
            mat[i][j] = isEnc ? sBox(mat[i][j]) : sBoxInv(mat[i][j]);
            // Use the correct sbox for encryption / decryption
        }
    }
}

void AESHandler::shiftRows(Matrix4x4 &mat, bool isEnc) {
    for (size_t i = 0; i < mat.size(); i++) {
        rotate(mat[i], isEnc ? i : -i); // if encrypting rotating left, if decrypting rotate right.
    }
}

void AESHandler::mixColumns(Matrix4x4 &mat) {
    for (int col = 0; col < 4; col++) {
        uint8_t s0 = mat[0][col];
        uint8_t s1 = mat[1][col];
        uint8_t s2 = mat[2][col];
        uint8_t s3 = mat[3][col];

        // Use lookup tables for multiplication by 2 and 3
        mat[0][col] = mul2[s0] ^ mul3[s1] ^ s2 ^ s3;
        mat[1][col] = s0 ^ mul2[s1] ^ mul3[s2] ^ s3;
        mat[2][col] = s0 ^ s1 ^ mul2[s2] ^ mul3[s3];
        mat[3][col] = mul3[s0] ^ s1 ^ s2 ^ mul2[s3];
    }
}

void AESHandler::mixColumnsInv(Matrix4x4 &mat) {
    for (int col = 0; col < 4; col++) {
        uint8_t s0 = mat[0][col];
        uint8_t s1 = mat[1][col];
        uint8_t s2 = mat[2][col];
        uint8_t s3 = mat[3][col];

        // Use lookup tables for multiplication by 9, 11, 13, 14
        mat[0][col] = mul14[s0] ^ mul11[s1] ^ mul13[s2] ^ mul9[s3];
        mat[1][col] = mul9[s0] ^ mul14[s1] ^ mul11[s2] ^ mul13[s3];
        mat[2][col] = mul13[s0] ^ mul9[s1] ^ mul14[s2] ^ mul11[s3];
        mat[3][col] = mul11[s0] ^ mul13[s1] ^ mul9[s2] ^ mul14[s3];
    }
}

void AESHandler::addRoundKey(Matrix4x4 &mat, uint8_t round) {
    for (size_t i = 0; i < BLOCK; i++) {
        ((uint8_t *) mat.data())[i] ^= keys[round][i];
    }
}

void AESHandler::xorEqual(Matrix4x4 &to, Matrix4x4 &with) {
    for (size_t i = 0; i < BLOCK; i++) {
        ((uint8_t *) to.data())[i] ^= ((uint8_t *) with.data())[i];
    }
}

void AESHandler::padData(vector<uint8_t> &data) {
    // Write the number of byte remained for multiplication of 16
    uint8_t padding = BLOCK - (data.size() % BLOCK);
    data.insert(data.end(), padding, padding);
}

void AESHandler::dePadData(vector<uint8_t> &data) {
    uint8_t padding = data[data.size() - 1];
    data.resize(data.size() - padding);
}

void AESHandler::keyExpansion(AESKey key) {
    // First round key is the original key
    keys[0] = key;

    // Generate the next 10 round keys
    for (size_t round = 1; round <= R; round++) {
        AESKey newKey;

        // Get the previous key for reference
        const auto &prevKey = keys[round - 1];

        // Handle the first word of the key specially
        Word temp;
        // Get last word of previous key
        for (size_t i = 0; i < WORD; i++)
            temp[i] = prevKey[BLOCK - WORD + i];

        // Apply key schedule core operations
        rotate(temp, 1); // Rotate word
        subWord(temp); // Apply S-box substitution
        temp[0] ^= rcon(round); // XOR with round constant

        // Generate first word of new key
        for (size_t i = 0; i < WORD; i++)
            newKey[i] = prevKey[i] ^ temp[i];

        // Generate remaining words
        for (size_t i = WORD; i < BLOCK; i++)
            newKey[i] = newKey[i - WORD] ^ prevKey[i];

        // Store the new key
        keys[round] = newKey;
    }
}

void AESHandler::encryptMat(Matrix4x4 &mat) {
    addRoundKey(mat, 0);
    for (size_t i = 1; i < R; i++) {
        subBytes(mat, true);
        shiftRows(mat, true);
        mixColumns(mat);
        addRoundKey(mat, i);
    }
    subBytes(mat, true);
    shiftRows(mat, true);
    addRoundKey(mat, R);
}

void AESHandler::decryptMat(Matrix4x4 &mat) {
    addRoundKey(mat, R);
    shiftRows(mat, false);
    subBytes(mat, false);
    for (size_t i = R - 1; i > 0; i--) {
        addRoundKey(mat, i);
        mixColumnsInv(mat);
        shiftRows(mat, false);
        subBytes(mat, false);
    }
    addRoundKey(mat, 0);
}

Matrix4x4 &AESHandler::convertToMat(uint8_t *dataStart) {
    return *reinterpret_cast<Matrix4x4 *>(dataStart);
}
